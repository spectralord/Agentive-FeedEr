...
| **Verifier (Prüfer)** | Zweistufige kritische Prüfung (Epic 10). **Stufe 1 (Reel-Verifier):** ein eigener Kritiker-Pass prüft ein Reel auf **Treue zur Quelle** und **Skepsis** (riskante Aussage-Typen) → erzeugt `caveat`. **Stufe 2 (Cluster-Korroboration):** leitet auf der Wissens-/Cluster-Ebene eine `confidence` aus der Zahl unabhängiger stützender Quellen ab. Kein „LLM entscheidet Wahrheit". |
| **caveat** | Ein gespeicherter Vorbehalt an einem Reel (Text, nullable), den der Reel-Verifier setzt, wenn die Aufbereitung die Quelle überzeichnet oder eine Aussage Vorsicht verdient. Wird als ⚠️ sichtbar gemacht und ist filterbar — **getrennt** vom `quality_score` (ADR 0004). |
| **confidence** | Vertrauensmaß auf der Wissens-/Cluster-Ebene: wie gut ein Claim/Thema durch **unabhängige** Quellen gestützt ist (Korroboration). Keine LLM-Wahrheitsbewertung, sondern ein Konsens-Signal. Hängt am Clustering (Vision V1). |
| **Korroboration** | Das Stützen eines Claims durch mehrere unabhängige Quellen. Grundlage der `confidence` — zuerst aus dem eigenen eingesammelten Korpus (Topic-Cluster), später optional per externer Web-Suche erweitert. |
| **Topic-Knowledge-Check** | Ein Feature (Epic 11) auf Basis Clustering, das pro **Topic-Cluster** aus *einem* Quervergleich über Quellen/Zeit zwei Ausgaben liefert: `confidence` (Korroboration) und `freshness` (Supersession). Vereint die frühere Verifier-Stufe-2 und den SOTA-Re-Check. Ergebnisse propagieren auf referenzierende Items (Skill-Nodes, Saves, SOTA). |
| **freshness / Supersession** | Erkennung, dass Neueres Älteres ablöst (z. B. ein umbenannter Parameter). Markiert Älteres via `superseded_by`/`lifecycle_state=deprecated`. Geerdeter Vergleich der Cluster-Items untereinander, keine externe Wahrheitsbewertung. |
