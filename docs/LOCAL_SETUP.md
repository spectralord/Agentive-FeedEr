...
**The app boots but every page is empty** — the seed didn't run. `npm run db:seed`. (Before the
split test database landed, `npm test` also emptied `feedr_dev`; it no longer does.)
