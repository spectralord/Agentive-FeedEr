...
**`npm test` fails on a fresh clone** — it shouldn't; tests are hermetic and do not read your
`.env`. If one does start depending on ambient environment, that's a bug in the test — mock the
env module instead (see `src/lib/admin/auth.test.ts` for the pattern).
