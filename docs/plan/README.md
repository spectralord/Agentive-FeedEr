...
| 22 — Writing assistance | `epic-22-writing-assistance.md` | Fast-Follow | 📋 **Plan fertig, delegierbar** (ADR 0026, 2026-08-03 gegrillt + akzeptiert, **auf einen Consumer verengt**). Der Grill hat die „zwei Oberflächen"-Prämisse geprüft: Experience-Report-Authoring **existiert und läuft** (`experience_reports.body`), Guide-Editing **nicht** (keine `skill_guides`-Tabelle, ADR 0018 baugesperrt). Modul bleibt wiederverwendbar, nur die UI wird verengt. Zwei Intents („improve", „shorten"), Vorschlag statt Ersetzung, Kontext = **nur der eigene Text** (damit ADR 0005 trivial erfüllt). Haupt-Constraint: `/experience/new` ist heute ein reines Server-HTML-Formular **ohne Client-JS** — nur das Body-Feld wird zur Client-Komponente, der native POST bleibt unangetastet |
