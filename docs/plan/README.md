...
| 20 — Actionables & Evidence-Track | `epic-20-actionables.md` | Fast-Follow | ✅ **fertig** (ADR 0019; T20.1–T20.5, Sonnet-Subagent 2026-08-01, Review durch das starke Modell). `actionable_completions` + Migration `0011`, `src/lib/actionables/` mit **einer** geteilten `toggleActionable`-Mutation (§8.4 — genau *ein* Insert-Pfad, *ein* HTTP-Endpunkt, *ein* geteiltes `ActionableTick`-Component für beide Oberflächen), Evidence-Counts in `getSkillMap`/`getNodeDetail`, To-Try-Liste mit `effort_tag`-Filter + Sortierung, Adoption-Log mit zweiter Quelle. Build/Typecheck grün, **436 Tests / 68 Files**, eslint 0. **Entscheidung-5-Regressionstest per Sabotage geprüft**: Snapshot entfernt → Test schlägt fehl (kein Test, der nur zufällig grün ist). **Kein Auto-Advance**: `toggleActionable` schreibt nie `user_progress`; der Vorschlag läuft über denselben expliziten Status-Submit. Rung-1 verifiziert (Ticks 40×40px, Overflow 0, „MASTERED/Declared" neben „0 ITEMS/Evidenced" — die Zwei-Spur-Trennung ist sichtbar korrekt) |
