...
/**
 * "🔁 Dranbleiben" (T6.5): a full-height card below the Top-N on /today
 ...
 * "erledigt" checkbox — items age out of the window naturally at 21 days, or
  ...
  return (
    <div className="reel flex min-h-dvh snap-start items-center justify-center [scroll-snap-stop:always]">
      <div className="mx-auto flex w-full max-w-xl flex-col gap-4 px-6">
        <h2 className="text-center text-lg font-semibold text-zinc-50">🔁 Dranbleiben</h2>
        <ol className="flex flex-col gap-3">
          {reels.map((reel) => (
            <li key={reel.id} className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-4">
              <p className="text-xs text-zinc-400">
                Vor {daysAgo(reel.savedAt, now)} Tagen gespeichert — nochmal ansehen?
              </p>
              <a
                href={reel.url}
                target="_blank"
