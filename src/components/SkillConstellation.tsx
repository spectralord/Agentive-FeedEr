...
/**
 * `/skills` (Epic 21, T21.4 / ADR 0020 stage a): skills as dots inside
 * hand-placed theme regions (`THEME_LAYOUT`), each at its `resolveNodePosition`
 * coordinate — the constellation view. Renders **alongside** the existing
 * grouped-list `SkillMap`, not instead of it (that view still works today
 * and stays; see the epic file's out-of-scope list) — `SkillsPage` decides
 * which is visible.
 *
...
export function SkillConstellation({ themes }: { themes: SkillMapTheme[] }) {
  const nodes = assignLabelRows(themes.flatMap((t) => t.nodes));

  return (
    <div className="relative mt-4 w-full overflow-hidden rounded-lg border border-hairline bg-surface">
      <div className="relative aspect-square w-full">
        ...
        {nodes.map((node, index) => (
          <Link
            ...
            // Epic 21 (T21.4): several nodes can hash close together inside
            // one theme circle (see resolveNodePosition/hashPositionInRegion's
            // spacing target — a *target*, not a hard guarantee, especially
            // at the current tiny corpus size). The ring itself always
            // renders at the exact resolved (x, y) — never moved — but its
            // *label* is placed via `assignLabelRows` (label-only collision
            ...
            // still lift a label above its neighbours via z-index, the same
            // trick real map labels use when two pins sit very close.
            // line-clamp-2 (not truncate — SkillMap.tsx's readability rule
            // applies here too) keeps the full title, never a hard cut.
            className="group absolute w-20 -translate-x-1/2 -translate-y-1/2 hover:z-20 focus-visible:z-20"
            style={{
