...
Binding rules:
- Output 5-8 short bullet points in German describing what the developer likes or skips, each phrased like "mag: …" or "überspringt: …" (e.g. "mag: konkrete Claude-Code-Beispiele", "überspringt: reine Ankündigungs-News ohne Substanz").
...
export const feedbackSummaryJsonSchema = {
  ...
  properties: {
    bullets: {
      ...
      maxItems: 8,
      description:
        '5-8 short German bullet points describing observed like/skip patterns, e.g. "mag: …" / "überspringt: …".',
    },
  },
  required: ["bullets"],
};
...
export function buildFeedbackUserPrompt(rows: RecentInteractionRow[]): string {
  const lines = rows.map(
    (r) =>
      `- ${TYPE_LABELS[r.type]}: "${r.title}" (Kategorie: ${r.category}, Skill: ${r.skill ?? "—"})`,
  );
  return ["## Letzte Interaktionen (neueste zuerst)", ...lines].join("\n");
}
